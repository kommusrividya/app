...
$sql = "SELECT EVENT_ID, DEShCode, Event_date, Event_Description FROM BSPD_Event where Event_status = 0 and EVENT_ID like 'CH%' order by Event_date desc;";
...
while($row = mysqli_fetch_array($query))
{
    ...
    $member = new stdClass();
    $member->sno = $row["SrNo"];
    $member->SrNo_EVent = $row["SrNo_EVent"];
...
where TABLE_NAME='Temp_BSPD_CashHandOver' and COLUMN_NAME != 'EVENT_ID' AND COLUMN_NAME != 'DEShCode'";
...
$sql = "SELECT * FROM Temp_BSPD_CashHandOver WHERE EVENT_ID = '$curr_event_id'";
$query = mysqli_query($link, $sql);
