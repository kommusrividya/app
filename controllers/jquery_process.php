...
require "$APPDIR/PHPMailer/src/Exception.php";
require "$APPDIR/PHPMailer/src/PHPMailer.php";
require "$APPDIR/PHPMailer/src/SMTP.php";
...
if (isset($_POST['bring_event_details'])) {
	...
	$sql = "SELECT * FROM BSPD_Event where EVENT_ID = '$search' and DEShCode = '$entity_id';";
	$query = mysqli_query($link, $sql);
	if (mysqli_num_rows($query) > 0) {
		$i = 0;
		while ($row = mysqli_fetch_assoc($query)) {
...
if (isset($_POST['event_registration_form'])) {
	...
	$sql = "SELECT * FROM BSPD_Event_Registration where DEShCode = '$entity_id' and EVENT_ID = '$event_id' and MEMBER_ID = $member_id;";
	$query = mysqli_query($link, $sql);
	if (mysqli_num_rows($query) > 0) {
		$sql1 = "UPDATE BSPD_Event_Registration SET `Registered` = '$reg', `UpdatedBy` = '".$_SESSION['id']."' WHERE (`MEMBER_ID` = '$member_id') and (`EVENT_ID` = '$event_id') and (`DEShCode` = '$entity_id');";
	...
	$sql = "INSERT INTO BSPD_Event_Registration (`MEMBER_ID`, `EVENT_ID`, `Registered`, `Attended`, `DEShCode`, `CreatedBy`, `UpdatedBy`) VALUES ('$member_id', '$event_id', '$reg', ' ', '$entity_id', '".$_SESSION['id']."', '".$_SESSION['id']."');";
	$query = mysqli_query($link, $sql);
...
if (isset($_POST["event_form"])) {
	...
	if ($form_mode == "create") {
		$sql = "select * from BSPD_Event where EVENT_ID = '$event_id' and DEShCode = '$entity_id';";
		...
		(`EVENT_ID`, `DEShCode`, `Event_date`, `Event_Description`, `Event_Location`,`Event_Notes`, `Event_status`) 
		VALUES ('$event_id', '$entity_id' , '$event_date', '$event_description', '$event_location', '$event_notes', '$event_status');";
...
if (isset($_POST["bank_reg"])) {
	...
	$sql = "SELECT * FROM BSPD_Payee_Account where Payee_ID = '$search';";
...
if (isset($_POST["expense_form"])) {
	...
	*/
	if ($form_mode == "create") {
		...
		$sql = "SELECT max(Voucher_Num)+1 as voucher FROM BSPD_Expenses where EVENT_ID='$event' and `DEShCode` = '$entity_id';";
		...
		`Payment_Confirmation_ID`, `UTR_Number`, `Note`, `DEShCode`) 
		...
		'$brc', '$payment_intimation_url', '$payment_date', '$payment_status', '$payment_confirmation_id', '$utr_number', '$notes', '$entity_id');";
...
if (isset($_POST['voucher_details'])) {
	...
	$voucher_num = $_POST['voucher_num'];
	$entity_id = $_POST["entity_id"];
	$sql = "SELECT * FROM BSPD_Expenses where EVENT_ID = '$search' and Voucher_Num = $voucher_num and DEShCode = '$entity_id';";
	$query = mysqli_query($link, $sql);
	if (mysqli_num_rows($query) > 0) {
		$i = 0;
		while ($row = mysqli_fetch_assoc($query)) {
...
if (isset($_POST['event_report_form'])) {
	...
	$sqlqry = "SELECT * FROM BSPD_Event_Registration where EVENT_ID = '$event_id' and DEShCode = '$entity_id';";
...
if (isset($_POST['contribution_processing_form'])) {
	...
	for ($i = 0; $i < $length; $i++) {
		...
		if ($event[$i] == "CANCEL") {
			...
			$sql1 = "INSERT INTO `BSPD_Member_Contribution` (`Member_id`, `EVENT_ID`, `Amount`, `Contribution_Type`, `Contribution_Date`, `Reference_Details`, `DEShCode`, `Approved`, `CreatedBy`) 
    		VALUES ('" . $row['Member_id'] . "', '" . $event[$i] . "', '" . $row['Amount'] . "', '" . $row['Contribution_Type'] . "', '" . $row['Contribution_Date'] . "', '" . $row['Reference_Details'] . "','". $entity_id[$i]. "' , 'Y', '1933');";

			if (mysqli_query($link, $sql1)) { 
				$transaction_id = mysqli_insert_id($link);
				$type="BSPDHYDContributionReceipt";
				$encrypted_id = EncryptDetails($link, $transaction_id, $type);
				$Receipt_PDF_URL = "https://www.bspd.in/app/receiptgenerate?id=$encrypted_id";
...
if (isset($_POST['permissions_form'])) {
	...
	$sql = "SELECT * FROM BSPD_Member where MEMBER_ID = $MEMBER_ID and Status = 'Active'";
	$query = mysqli_query($link, $sql);
	if (mysqli_num_rows($query) == 0) {
...
if (isset($_POST['event_attendance_form'])) {
	...
	$sql = "SELECT * FROM BSPD_Event_Registration where MEMBER_ID = $MEMBER_ID and EVENT_ID = '$event_id' and DEShCode = '$entity_id';";
	$query = mysqli_query($link, $sql);

	if (mysqli_num_rows($query) > 0)
		$sql = "UPDATE BSPD_Event_Registration SET `Attended` = 'Y' WHERE (`MEMBER_ID` = '$MEMBER_ID') and (`EVENT_ID` = '$event_id') and (`DEShCode` = '$entity_id');";
	else {
		$sql = "INSERT INTO BSPD_Event_Registration (`MEMBER_ID`, `EVENT_ID`, `DEShCode`, `Registered`, `Attended`) VALUES ('$MEMBER_ID', '$event_id', '$entity_id', '', 'Y');";
...
if (isset($_POST['event_exp_report_form'])) {
	...
	if ($report == "recognition") {
		$sqlqry1 = "Select * from BSPD_View_Recognition where Event_ID = '$event_id' order by SerialNum";
		$result = mysqli_query($link, $sqlqry1);

		$output .= "<table border='1' style='border-collapse: collapse' class='table table-condensed'>";
		$output .= "<th>SNo</th><th>Member Name</th><th>Recognition</th><th>Recognition Desc</th>";
		$i = 1;
		while ($row = mysqli_fetch_array($result)) {
		    
			$output .= "<tr><td>".$i."</td><td>" . $row['Alias'] . "</td><td>" . $row['Recognition'] . "</td><td>" . $row['Notes'] . "</td></tr>";
			$i++;
		}
		echo $output;
	}
...
if (isset($_POST['password_reset_form'])) {
	...
	if(mysqli_num_rows($query) > 0)
	{
		...
		if(mysqli_query($link, $sql1))
		{
			...
			$header = 'New Pass '. $pwd." for ".$row['Alias'];
			$From = 'From: bspd.hyd@gmail.com';
			$Message = "Namaste ".$row["Alias"]." garu, <br>";
			$Message .= "Ph: ".$row["Phone_Num"]."<br>";
			$Message .= "You have requested a password reset on BSPD Self Service Site.<br>";
			$Message .= "Please use the pin provided below for login.<br><br>";
			$Message .= "PIN: $pwd <br><br>";

			$Message .= "Request you to change your password after logging in.<br><br>";
			$Message .= "Regards,<br>Team BSPD";
			
			
// 			$this_mail = mail($to, $header, $Message, $From);
            sendanemail($to, $header, $Message);
            
            echo "<br>Your new password ".substr($pwd,0,2)."XXXX";
...
if(isset($_POST['cash_collection_form'])) {
    ...
    $entity_id = mysqli_real_escape_string($link, $_POST["entity_id"]);
    ...
    if($sno == "New") {
        $sql1 = "SELECT MAX(SrNo_EVent) AS SrNo_EVent FROM Temp_BSPD_GNCD_Log WHERE EVENT_ID = '$event_id'";
        $query1 = mysqli_query($link, $sql1);
        $res = mysqli_fetch_array($query1)['SrNo_EVent'];
        $res = ($res === NULL) ? 1 : (int)$res + 1;

        $sql = "INSERT INTO Temp_BSPD_GNCD_Log 
        (`EVENT_ID`, `SrNo_EVent`, `DEShCode`, `Collector_ID`, `Contributer_ID`, `Amount`, `Notes`, `Status`) 
        VALUES ('$event_id', $res, '$entity_id', '".$_SESSION['id']."', '$memberid', '$amount', '$note', 'entered')";
    } else {
        $sno = (int)$sno;
        $sql = "UPDATE Temp_BSPD_GNCD_Log SET 
        `EVENT_ID` = '$event_id', 
...
if(isset($_POST['preview'])) {
	$sql = "SELECT * FROM Temp_BSPD_GNCD_Log WHERE Status = 'entered' order by SrNo";
	$result = mysqli_query($link, $sql);
	$output = "";
	$i = 1;
	...
	while($row = mysqli_fetch_array($result)) {
	    $minutes_to_add = 750;
		$time = new DateTime($row['CreatedDate']);
		$time->add(new DateInterval('PT' . $minutes_to_add . 'M'));
		$output .= "<tr>";
		...
		$output .= "<td>CASH</td>";
		$output .= "<td>".$time->format('Y-m-d H:i:s')."</td>";
...
if(isset($_POST['cash_record_generate'])) {
	...
	while($row = mysqli_fetch_array($result)) {
		...
		$time = new DateTime($row['CreatedDate']);
		$time->add(new DateInterval('PT' . $minutes_to_add . 'M'));
		$sql1 = "INSERT INTO BSPD_Member_Contribution (`Member_id`, `DEShCode`, `EVENT_ID`, `Amount`, `Contribution_Type`, `Contribution_Date`, `Reference_Details`, `Approved`, `CreatedBy`) 
    		VALUES ('" . $row['Contributer_ID'] . "', '".$row['DEShCode']."' , '" . $row['EVENT_ID'] . "', '" . $row['Amount'] . "', 'CASH', '" . $time->format('Y-m-d H:i:s') . "', 'CASH DESK " . str_pad($i,3,"0",STR_PAD_LEFT) . "', 'Y', '".$_SESSION['id']."');";
			...
			if (mysqli_query($link, $sql1)) { 
				...
				$sql3 = "UPDATE Temp_BSPD_GNCD_Log SET Status = 'reconciled' WHERE SrNo = ".$row['SrNo'].";";
...
if(isset($_POST['deletemembercontribution'])) {
	...
	$sql = "DELETE FROM Temp_BSPD_GNCD_Log where SrNo = '$sno'";
	$query = mysqli_query($link, $sql);
...
if(isset($_POST['link_utr_form'])) {
	...
	for($i=0;$i<$length;$i++)
	{
		$sql = "UPDATE Temp_BSPD_GNCD_Log SET `UTR` = '$utr', `Status` = 'UTRlinked' WHERE (`SrNo` = '$snos[$i]');";
		$query = mysqli_query($link, $sql);
		if($query) $message .= "utr change for $snos[$i] successful\n";
		else $message .= mysqli_error($link);
	}
	echo $message;
}
...
if(isset($_POST['utrsave'])) {
	...
	$sql = "UPDATE Temp_BSPD_GNCD_Log SET `UTR` = '$newutr' WHERE (`UTR` = '$utr');";
	$query = mysqli_query($link, $sql);
...
if(isset($_POST['utrdelete'])) {
	...
	$sql = "UPDATE Temp_BSPD_GNCD_Log SET `UTR` = '0' , `Status` = 'entered' WHERE (`UTR` = '$utr');";
	$query = mysqli_query($link, $sql);
...
if(isset($_POST['generatereceipt'])) {
	...
	if( $sumcontribution != $bank_amount ) { echo "Amount mismatch."; return; }
		...
		$sql4 = "UPDATE Temp_BSPD_GNCD_Log set Status = 'reconciled' where UTR = '$utr';";
		if(mysqli_query($link, $sql4))	$message .= "Marked records as reconciled";
		else $message .= mysqli_error($link)."\n";
...
if (isset($_POST['save_denomination'])) {
    ...
    $sql    = "SELECT * FROM `Temp_BSPD_CashHandOver` WHERE EVENT_ID = '$event_id'";
    $result = mysqli_query($link, $sql);

    if (!$result) {
        die("❌ Query Failed: " . mysqli_error($link) . " | SQL: " . $sql);
    }
    ...
    if ($rowCount > 0) {
        ...
        $sql = "UPDATE `Temp_BSPD_CashHandOver`
                   SET `N500` = '".$qty_arr[0]."',
                       `N200` = '".$qty_arr[1]."',
                       `N100` = '".$qty_arr[2]."',
                       `N50`  = '".$qty_arr[3]."',
        ...
        if (mysqli_query($link, $sql)) {
            echo "✅ Denominations updated successfully<br>";
        } else {
            echo "❌ Update error: " . mysqli_error($link) . "<br>";
        }
    }

    // 3. If not exists → INSERT
    else {
        echo "➡️ No row found, performing INSERT...<br>";

        $sql = "INSERT INTO `Temp_BSPD_CashHandOver`
                   (`DEShCode`, `EVENT_ID`, `N500`, `N200`, `N100`, `N50`, `N20`, `N10`, `N5`, `N2`, `N1`, `C10`, `C5`, `C2`, `C1`)
                VALUES
                   ('$entity_id', '$event_id',
                    '".$qty_arr[0]."', '".$qty_arr[1]."', '".$qty_arr[2]."', '".$qty_arr[3]."',
                    '".$qty_arr[4]."', '".$qty_arr[5]."', '".$qty_arr[6]."', '".$qty_arr[7]."',
                    '".$qty_arr[8]."', '".$qty_arr[9]."', '".$qty_arr[10]."', '".$qty_arr[11]."',
                    '".$qty_arr[12]."')";
                    
        echo "sql : $sql";

        if(mysqli_query($link, $sql)) {
            echo "✅ Denominations inserted successfully<br>";
        } else {
            echo "❌ INSERT failed: " . mysqli_error($link) . "<br>SQL: $sql<br>";
        }
    }
}
...
if(isset($_POST['recognition'])) {
    ...
    $sql = "SELECT 
                *
            FROM
                `BSPD_View_Names_4_Seshavastra` `S` JOIN `BSPD_Event_Registration` `R`
            WHERE
            	S.MEMBER_ID  = R.MEMBER_ID AND
            	R.Attended = 'Y' AND
                R.EVENT_ID = 'CH0090' AND
                S.Spouse_ID > 1000 AND
                S.Current_Age > 50
            ORDER BY
            	S.Aten_count desc,
            	S.Current_Age desc;
    ...
    $recognition_attended = array();
    
    while($row = mysqli_fetch_array($result)) {
        $recognition_attended[] = $row;
    }
    
    $sql = "SELECT 
                *
            FROM
                `BSPD_View_Names_4_Seshavastra`
            WHERE
                Spouse_ID > 1000
                    AND MEMBER_ID NOT IN (SELECT 
                        S.MEMBER_ID
                    FROM
                        `BSPD_View_Names_4_Seshavastra` `S`
                            JOIN
                        `BSPD_Event_Registration` `R`
                    WHERE
                        S.MEMBER_ID = R.MEMBER_ID
                            AND R.Attended = 'Y'
                            AND R.EVENT_ID = 'CH0090'
                            AND S.Spouse_ID > 1000
                            AND S.Current_Age > 50
                    ORDER BY S.Aten_count DESC , S.Current_Age DESC)
            ORDER BY Aten_Count DESC;";
    ...
    $response['recognition_attended'] = $recognition_attended;
...
if(isset($_POST['insert_post'])) {
    ...
    $opp_header = $_POST['opp_header'];
    $opp_description = $_POST['opp_description'];
    $skill_keywords = $_POST['skill_keywords'];
    $years_of_experience = $_POST['years_of_experience'];
    $job_type = $_POST['job_type'];
    
    $sql = "INSERT INTO Career_Opportunity (posted_by, created_at, updated_at, company, contact_phno, contact_email, contact_notes, opp_header, opp_description, skill_keywords, years_of_experience, job_type) 
            VALUES (
                '{$postedby}', 
                NOW(), 
                NOW(), 
    ...
    if ($result) {
        ...
        echo "New Post ID: " . $newPostId;
    } else {
        echo "Failed to create post.";
    }
}

function sendanemail($receiver, $subject, $content) {
...
try {
    ...
    $mail->send();
    echo 'Message has been sent to XXX'.substr($receiver,3, strlen($receiver)). " <h3><b>Mail may take up to <b>2 minutes<b> to reach your inbox.</b></h3>";
} catch (Exception $e) {
