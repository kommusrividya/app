...
$sql = "SELECT EVENT_ID, DEShCode, Event_date, Event_Description FROM BSPD_Event where Event_status = '0' order by Event_date desc;";
...
while($row = mysqli_fetch_array($result))
{
    ...
    if($row1["voucher"] != NULL) $event->voucher_num = $row1["voucher"];
    else $event->voucher_num = 1;
    $events[] = $event;
}
...
$sql = "SELECT distinct Sub_Category_ID, Sub_Category_Desc FROM BSPD_Transaction_Code_Master where Category_ID = 1 and Categroy_Type = 'Expense';";
$result = mysqli_query($link, $sql);
