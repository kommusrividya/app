...
$receipt->entity_id = $row['DEShCode'];
$receipt->event_loc = $row['Event_Location'];
$receipt->ref = $row['Reference_Details'];

$sql1 = "SELECT Notes FROM BSPD_SIB_Collection_Report WHERE SLNO = '".$receipt->ref."';";
$query = mysqli_query($link, $sql1);
$row = mysqli_fetch_array($query);
$notes = $row['Notes'];

$charno = 75;
$original_string = "Received from Smt./Shri. ".$receipt->name." ( Member Id ".$receipt->member_id." ) a sum of Rs. ". $receipt->amount ." ( ". $receipt->amount_in_words ." ) through ". $receipt->cont_type ." contribution towards ". $receipt->event_desc ." ( ".$receipt->entity_id." ". $receipt->event_id ." ) at ". $receipt->event_loc ." vide Reference : ( ". $receipt->ref ." ".$notes." ) Dated: ". $receipt->cont_date;
