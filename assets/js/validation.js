...
$(document).ready(function () {
    if($("#couple_rec").length) populateRecognition();
        ...
        if (form_mode == "update") {
            $.ajax({
                ...
                data: {
                    ...
                    "entity_id": $("#event_id").find(':selected').data('entity_id'),
    ...
    $("#event_form").submit(function (event) {
        ...
        event.preventDefault();
        let entity_id = "";
        if($("#form_mode").val() == "create") entity_id = $("#entity_id").val();
        else entity_id = $("#event_id").find(':selected').data('entity_id');
    ...
    $("#expense_edit").on('click', function (event) {
        ...
        $.ajax({
            ...
            data: {
                "event": $("#event").val(),
                "entity_id": $("#event").find(':selected').data('entity_id'),
                "voucher_num": $("#voucher_num").val(),
                "voucher_details": 1,
            },
    ...
    $("#event_report_form").submit(function (event) {
        ...
        var formdata = {
            ...
            entity_id: $("#report_event_id").find(':selected').data('entity_id'),
    ...
    $("#contribution_processing_form").submit(function (event) {
        ...
        if (answer) {
            ...
            for (i = 0; i < $("#count").val(); i++) {
                event[i] = $("#event" + i).val();
                entity[i] = $("#event" + i).find(':selected').data('entity_id'),
                SrNo[i] = $("#SrNo" + i).val();
            }
    ...
    $("#event_attendance_form").on('click', function (event) {
        ...
        var formdata = {
            MEMBER_ID: $("#MEMBER_ID").val(),
            entity_id: $("#attendance_event_id").find(':selected').data('entity_id'),
    ...
    $("#insertmembercontribution").on('click', function(event){
        event.preventDefault();
        var number = $('input[name="membersno"]:checked').val();
        if(!number) { alert("Select a transaction to edit"); return; }
    ...
    $("#shift_contribution").on('click', function (event){
        event.preventDefault();
        
        var formdata = {
            shift_from: $("#shift_from").val(),
            shift_to: $("#shift_to").val(),
            curr_event_id: $("#curr_event_id").val(),
            shift_contribution: 1,
        };
    ...
    $("#fin_sum_form").on('submit', function (event) {
        ...
        let provisioned = Number($("#provisioned").val());
        ...
        $("#display").val(display);
        $("#future_exp").val(bank_balance-(reserves+pending_payments+provisioned));
    });
    ...
    $("#calctarabalam").on('click', function (event){
        event.preventDefault();
        let janma = +$("#janmanakshatra").val();
        let daynak = +$("#daynakshatra").val();
        console.log(janma);
        console.log(daynak);
    });
    ...
    $(".recog_action").on('click', function (event) {
        event.preventDefault();
        let id = $(this).attr('id');
...
function populateRecognition() {
        $.ajax({
            ...
            success: function (response) {
                console.log(response);
                let recog_at = response['recognition_attended'];
                let recog = response['recognition'];
                
                for (var i = 0; i < recog_at.length; i++) {
                    var table = document.getElementById("couple_rec");
                    var newRow = table.insertRow();
        
                    var cell2 = newRow.insertCell();
                    cell2.innerHTML = recog_at[i].Alias + " (" + recog_at[i].Phone_Num + ")";
        
                    var cell3 = newRow.insertCell();
                    cell3.innerHTML = recog_at[i].SpouseAlias;
        
                    var cell3 = newRow.insertCell();
                    cell3.innerHTML = recog_at[i].Aten_Count;
        
                    var cell3 = newRow.insertCell();
                    cell3.innerHTML = recog_at[i].Current_Age;
                    var cell3 = newRow.insertCell();
                    cell3.innerHTML = "<button class='recog_action' id='"+ recog_at[i].MEMBER_ID +"'>Add</button>";
                }
                
                for (var i = 0; i < recog.length; i++) {
                    var table = document.getElementById("t30_rec");
                    var newRow = table.insertRow();
        
                    var cell2 = newRow.insertCell();
                    cell2.innerHTML = recog[i].Alias;
        
                    var cell3 = newRow.insertCell();
                    cell3.innerHTML = recog[i].SpouseAlias;
        
                    var cell3 = newRow.insertCell();
                    cell3.innerHTML = recog[i].Aten_Count;
        
                    var cell3 = newRow.insertCell();
                    cell3.innerHTML = recog[i].Current_Age;
                }
                // location.reload();
            },
        });
